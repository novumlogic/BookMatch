        ...
        if (userLogged) {
            ...
            if (categoriesShown) {
                ...
                LaunchedEffect(reloadRecommendations) {
                    ...
                    if (reloadRecommendations) {
                        ...
                        recommendationStatus = withContext(dispatcherIO) {
                            viewModel.performRecommendationTask(context, selectedCategories, lastRecommendationTime)
                        }
                ...
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    ...
                    var currentRecommendationTimestamp by remember(lastRecommendationTime) {
                        mutableLongStateOf(
                            lastRecommendationTime
                        )
                    }
                    val appContentUiState = AppContentUiState(
                    ...
                    val appContentCallbacks = object : AppContentCallbacks{
                        override suspend fun fetchRecommendationTimestamp(): List<Recommendations>? {
                            return viewModel.fetchRecommendationTimestamp()
                        }

                        override fun fetchBooksFromRecommendationId(
                            recommendationId: Int,
                            recommendationTimestamp: Long
                        ) {
                            recommendationStatus = RecommendationStatus.LOADING
                            scope.launch(dispatcherIO) {
                                recommendationStatus = viewModel.fetchBooksFromRecommendationId(recommendationId,recommendationTimestamp)
                                currentRecommendationTimestamp = recommendationTimestamp
                            }
                        }

                        override fun filterEnabled(): Boolean {
                            return recommendationStatus != RecommendationStatus.LOADING
                        }

                        override fun onCategoryChipChange(categoryName: String, selected: Boolean) {
                            if (selected) {
                        ...
                        override fun onCategoryScreenContinue(): Boolean {
                            return if (selectedCategoryList.size < 2 || selectedCategoryList.size > 5) {
                                Toast.makeText(
                                    context,
                        ...
                        override fun onRecommendMore(
                            likedBookList: List<String>,
                            dislikedBookList: List<String>,
                            ...
                            scope.launch(dispatcherIO) {
                                ...
                                recommendationStatus =
                                    viewModel.recommendMore(likedBookList, dislikedBookList, ratings, totalBooks , selectedCategories ,currentTime) {
                                        viewModel.setLastRecommendationTime(context, currentTime)
                                    }
                                viewModel.failedReason = FailedReason.AI_ERROR
